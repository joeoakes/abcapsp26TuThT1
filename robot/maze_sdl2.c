...
// Includes ROS bridge forwarding: each autoplay move is also POST'd to MAZE_ROS_URL.
//
...
// ROS bridge endpoint (NEW)
static const char *g_ros_bridge_url = NULL;
...
static const uint8_t* glyph5x7(char c) {
    ...
    switch (c) {
        ...
        case 'T': return T; case 'U': return U; case 'Y': return Y;
        case ':': return colon; case ' ': return blank; default: return blank;
    }
}
...
static void draw_dashboard_overlay(SDL_Renderer* r, int win_w, int win_h, bool won_local) {
    ...
    if (!mission_active) {
        if (won_local) { headerR = 63; headerG = 185; headerB = 80; }
        else { headerR = 248; headerG = 81; headerB = 73; }
    }
...
/*
 * send_to_ros_bridge()
 * Fire-and-forget POST to the ROS bridge service.
 * Uses a short 600 ms timeout so it never stalls the game loop.
 * Has no effect if MAZE_ROS_URL is not set.
 */
static void send_to_ros_bridge(const char* action, int px, int py) {
    if (!g_ros_bridge_url) return;

    char json[256];
    snprintf(json, sizeof(json),
        "{\"action\":\"%s\",\"session_id\":\"%s\",\"x\":%d,\"y\":%d}",
        action, session_id, px, py
    );

    (void)http_post_json(g_ros_bridge_url, json, NULL, 0, 600L);
}
...
static bool autoplay_step(int* px, int* py, int* move_sequence, bool* won) {
    ...
    // ---- ROS bridge: determine action string from dx/dy and forward ----
    const char* ros_action = "UP";
    if (dx == 1)       ros_action = "RIGHT";
    else if (dx == -1) ros_action = "LEFT";
    else if (dy == 1)  ros_action = "DOWN";
    send_to_ros_bridge(ros_action, *px, *py);
    // --------------------------------------------------------------------
    ...
    if (goal && mission_active) {
        *won = true;
        ...
        mission_active = false;
        send_to_ros_bridge("DONE", *px, *py);   // signal robot to stop
    }
...
int main(int argc, char** argv) {
    ...
    // ROS bridge endpoint (NEW)
    g_ros_bridge_url = getenv("MAZE_ROS_URL");
    if (!g_ros_bridge_url) g_ros_bridge_url = "http://127.0.0.1:5050/move";
    ...
    while (running) {
        ...
        draw_maze(renderer);
        draw_player_goal(renderer, px, py);
        draw_mode_hud(renderer);

        if (g_in_dashboard && g_draw_dashboard_overlay) {
            draw_dashboard_overlay(renderer, win_w, win_h, won);
        }
